...
*/
  ...
  setEnd: function (refNode, offset, bSafe, bNoUpdate) {
    ...
    if (!bNoUpdate) {
      if (
        !this.startContainer ||
        this._compareEndPoint(this.startContainer, this.startOffset, this.endContainer, this.endOffset) != -1
      ) {
        this.collapse(false);
      } else {
        this._updateRangeInfo();
    ...
    if (oStartContainer == oEndContainer && iStartOffset == iEndOffset) {
      ...
      if (
        ...
        (this._getNextNode(oActualEndNode) == oActualStartNode ||
          (oActualEndNode == oActualStartNode && this._isBlankTextNode(oActualEndNode)))
      ) {
        bCollapsed = true;
      }
    }
      ...
      if (oStart.oLineBreaker && oStart.oLineBreaker.nodeName === 'P' && (bBlock || clone.rxHasBlock.test(sHTML))) {
        ...
        if (elNextLine.innerHTML === '') {
          ...
          elNextLine.innerHTML =
            jindo.$Agent().navigator().ie && jindo.$Agent().navigator().version > 8 ? '\u200B' : '\uFEFF';
        }
