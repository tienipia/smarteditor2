...
export const HuskyCore = jindo.$Class({
  name: 'HuskyCore',
  aCallerStack: null,
  ...
  $init: function (htOptions) {
    ...
    this.appStatus = APP_STATUS.NOT_READY;
  ...
  exec: function (msg, args, oEvent) {
    ...
    if (this.appStatus == APP_STATUS.NOT_READY) {
  ...
  _exec: function (msg, args, oEvent) {
    return (this._exec = this.htOptions.oDebugger ? this._execWithDebugger : this._execWithoutDebugger).call(
      this,
      msg,
      args,
      oEvent
  ...
  registerPlugin: function (oPlugin) {
    ...
    if (oPlugin.status != PLUGIN_STATUS.NOT_READY) {
      oPlugin.status = PLUGIN_STATUS.READY;
    }
  ...
  run: function (htOptions) {
    ...
    this._changeAppStatus(APP_STATUS.WAITING_FOR_PLUGINS_READY);
  ...
  _changeAppStatus: function (appStatus) {
    ...
    if (this.appStatus == APP_STATUS.READY) {
  ...
  _waitForPluginReady: function () {
    ...
    for (var i = 0; i < this.aPlugins.length; i++) {
      if (this.aPlugins[i].status == PLUGIN_STATUS.NOT_READY) {
        bAllReady = false;
        break;
      }
    }
    if (bAllReady) {
      this._changeAppStatus(APP_STATUS.READY);
    } else {
  ...
  _loadLazyFiles: function (sMsg, aArgs, oEvent, aFilenames, nIdx) {
    ...
    if (_htLoadedFile[sFilename]) {
      ...
      jindo.LazyLoading.load(
        SE2M_Configuration.LazyLoad.sJsBaseURI + '/' + sFilename,
        jindo
 ...
 */
HuskyCore.mixin = function (oClass, htMixin, bOverride) {
  //TODO: error handling?
