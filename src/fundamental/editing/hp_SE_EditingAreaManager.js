 ...
 */
export const SE_EditingAreaManager = jindo.$Class({
  ...
  $init: function (sDefaultEditingMode, elContentsField, oDimension, fOnBeforeUnload, elAppContainer) {
    ...
    var oWidth = this._getSize(
      [oDimension.nWidth, oDimension.width, this.elEditingAreaContainer.offsetWidth],
      this.nMinWidth
    );
    var oHeight = this._getSize(
      [oDimension.nHeight, oDimension.height, this.elEditingAreaContainer.offsetHeight],
      this.nMinHeight
    );
  ...
  $BEFORE_MSG_APP_READY: function (/*msg*/) {
    this.oApp.exec('ADD_APP_PROPERTY', ['version', SE_EditingAreaManager.version]);
  ...
  $ON_IE_FOCUS: function () {
    ...
    if (!oAgent.ie && !oAgent.edge) {
      // [SMARTEDITORSUS-2257] edge도 ie와 동일하게 포커스처리해준다.
      return;
    }
  ...
  $ON_SHOW_EDITING_AREA_COVER: function (bDimmed) {
    if (!this.elEditingAreaCover) {
      ...
      this.elEditingAreaCover.style.cssText =
        'position:absolute;top:0;left:0;z-index:100;background:#000000;filter:alpha(opacity=0);opacity:0.0;-moz-opacity:0.0;-khtml-opacity:0.0;height:100%;width:100%';
  ...
  _convertLastBrToNbsp: function () {
    var elBody = this.oApp.getWYSIWYGDocument().body,
      ...
      elBrContainer,
      elImgContainer,
      elNextToImgContainer,
      elUndesiredContainer,
      aImg,
      elImg,
      nImgWidth,
      elImgParent,
