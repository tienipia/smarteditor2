 ...
 */
export const SE2M_FontNameWithLayerUI = jindo.$Class({
  ...
  _assignHTMLElements: function (elAppContainer) {
    ...
    this.aLIFontNames = jindo.$A(jindo.$$('LI', this.oDropdownLayer)).filter(function (v) {
  ...
  $ON_EVENT_FONTNAME_LAYER_CLICKED: function (wev) {
    ...
    var htFontInfo = this.htAllFonts[sFontFamily.replace(/"/g, SE2M_FontNameWithLayerUI.CUSTOM_FONT_MARKS)];
  ...
  $ON_SET_FONTFAMILY: function (sFontFamily, sDefaultSize) {
    ...
    // [SMARTEDITORSUS-169] 웹폰트의 경우 fontFamily 에 ' 을 붙여주는 처리를 함
    var oFontInfo = this.htAllFonts[sFontFamily.replace(/"/g, SE2M_FontNameWithLayerUI.CUSTOM_FONT_MARKS)];
    if (oFontInfo) {
  ...
  _getMatchingLI: function (sFontName) {
    ...
    for (var i = 0; i < this.aLIFontNames.length; i++) {
      ...
      for (var h = 0; h < aFontFamily.length; h++) {
        if (!!aFontFamily[h] && jindo.$S(aFontFamily[h].replace(/['"]/gi, '')).trim().$value() == sFontName) {
          return elLi;
        }
      }
    }
  ...
  addFont: function (fontId, fontName, defaultSize, fontURL, fontCSSURL, fontType, sSampleText, bCheck) {
    ...
    if (!fontType) {
      sFontFamily =
        SE2M_FontNameWithLayerUI.CUSTOM_FONT_MARKS + sFontFamily + SE2M_FontNameWithLayerUI.CUSTOM_FONT_MARKS;
    }
...
SE2M_FontNameWithLayerUI.CUSTOM_FONT_MARKS = "'"; // [SMARTEDITORSUS-169] 웹폰트의 경우 fontFamily 에 ' 을 붙여주는 처리를 함
