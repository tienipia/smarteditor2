 ...
 */
export const SE2M_FontSizeWithLayerUI = jindo.$Class({
  ...
  _assignHTMLElements: function (elAppContainer) {
    ...
    this.aLIFontSizes = jindo.$A(jindo.$$('LI', this.oDropdownLayer)).filter(function (v) {
  ...
  $ON_MSG_APP_READY: function () {
    ...
    for (var i = 0; i < this.aLIFontSizes.length; i++) {
      this.oApp.registerBrowserEvent(this.aLIFontSizes[i], 'click', 'SET_FONTSIZE', [
        this._getFontSizeFromLI(this.aLIFontSizes[i])
      ]);
    }
  },
