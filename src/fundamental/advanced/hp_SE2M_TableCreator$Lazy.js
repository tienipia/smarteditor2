...
HuskyCore.mixin(nhn.husky.SE2M_TableCreator, {
  ...
  $LOCAL_BEFORE_FIRST: function () {
    ...
    this.oApp.exec('TABLE_SET_BORDER_COLOR', [
      /#[0-9A-Fa-f]{6}/.test(this.elText_borderColor.value) ? this.elText_borderColor.value : '#cccccc'
    ]);
    this.oApp.exec('TABLE_SET_BGCOLOR', [
      /#[0-9A-Fa-f]{6}/.test(this.elText_BGColor.value) ? this.elText_BGColor.value : '#ffffff'
    ]);
  ...
  $ON_TABLE_INSERT: function () {
    ...
    var sTable, elLinebreak, elBody, elTmpDiv, elTable, elFirstTD, oSelection, elTableHolder, htBrowser;
    ...
    if (htBrowser.firefox) {
      ...
      if (document.documentMode == 11) {
        // [SMARTEDITORSUS-2064] IE11인 경우 빈 P태그 안에 커서가 안들어가기 때문에 br 추가
  ...
  _divideParagraph: function (oSelection) {
    var oParentP, welParentP, sBM, oSWrapper, oEWrapper;
