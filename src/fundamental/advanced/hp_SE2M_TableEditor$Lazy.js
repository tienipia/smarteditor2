...
HuskyCore.mixin(nhn.husky.SE2M_TableEditor, {
  ...
  $ON_TABLE_QE_SELECT_TEMPLATE: function (weEvent) {
    ...
    this.oApp.exec('RECORD_UNDO_ACTION', [
      'CHANGE_TABLE_STYLE',
      { elSaveTarget: elSaveTarget, sSaveTarget: sSaveTarget, bDontSaveSelection: true }
    ]);
  },
  ...
  $ON_TABLE_QE_SET_IMG: function (elSelected) {
    ...
    if (n === '0') {
      ...
      var sBackgroundImage = SE2M_Configuration.LinkageDomain.sCommonStatic + '/static/img/' + sImageName;
      for (i = 0, nLen = this.aSelectedCells.length; i < nLen; i++) {
  ...
  $ON_TE_SPLIT_COLUMN: function () {
    ...
    for (i = 0, nLen = this.aSelectedCells.length; i < nLen; i++) {
      ...
      var htBrowser = jindo.$Agent().navigator();
      if (
        (htBrowser.edge && Math.floor(htBrowser.version) === 12) ||
        (htBrowser.ie &&
  ...
  $ON_EVENT_EDITING_AREA_MOUSEMOVE: function (wevE) {
    ...
    if (htBrowser.ie && elTable && elTable.tagName && elTable.tagName.toUpperCase() === 'TABLE') {
      // [IE]
      if (sPointerUpEvent in elTable) {
        // [IE 11] resizeend 이벤트 deprecated
  ...
  _reassignCellSizes: function (elTable) {
    ...
    // [SMARTEDITORSUS-2136]
    var nTableWidth = elTable._nWidth,
      nTableHeight = elTable._nHeight,
      nResizedTableWidth = elTable._nResizedWidth,
      nResizedTableHeight = elTable._nResizedHeight, // [IE] resizeend 이벤트가 발생했다면 존재
      nTableWidthAdjustRatio = 1,
    ...
    (elTable._nWidth = welTable.width()), (elTable._nHeight = welTable.height());
    // --[SMARTEDITORSUS-2136]
  ...
  _fnOnMouseMoveResizeCover: function (oEvent) {
    ...
    if (this.htResizing.nResizeMode == 1) {
      this.elResizeGrid.style.left =
        oEvent.pos().clientX - this.nClientXDiff - this.parseIntOr0(this.elResizeGrid.style.width) / 2 + 'px';
    } else {
      this.elResizeGrid.style.top =
        oEvent.pos().clientY - this.nClientYDiff - this.parseIntOr0(this.elResizeGrid.style.height) / 2 + 'px';
    }
    // --[SMARTEDITORSUS-1504]
  },
  ...
  _markResizedMetric: function (elTable) {
    ...
    (elTable._nResizedWidth = welCurrentTable.width()), (elTable._nResizedHeight = welCurrentTable.height());
  },
  ...
  $ON_CLOSE_QE_LAYER: function (oEvent) {
    ...
    var elTarget = oEvent ? oEvent.element : null,
      ...
      aQEToggleClassName = ['q_open_table_fold', 'q_open_table_full'],
      sQEToggleClassName,
      i,
      len = aQEToggleClassName.length,
    ...
    switch (nNewStatus) {
      case this.STATUS.S_0:
        ...
        if (this.sQEAction) {
          this.oApp.exec('RECORD_UNDO_ACTION', [
            this.sQEAction,
            { elSaveTarget: this.elSelectionStartTable, bDontSaveSelection: true }
          ]);
          this.sQEAction = '';
        }
